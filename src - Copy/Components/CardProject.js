...
export default function CardProject(props) {
  ...
  useEffect(() => {
    if (props.currentTheme === "light") {
      setBackground('card-body bg-cyan-200 transition-colors duration-300');
      setHeading('card-text font-sans text-2xl text-slate-950 font-bold flex justify-center');
    } else {
      setBackground('card-body bg-black transition-colors duration-300');
  ...
  return (
    <div className="inline my-4">
      <div className="card border-0" style={{ width: '18rem' }}>
        <img src={props.data.image} style={{ width: '100%', height: '0%' }} className="card-img-top" alt="project thumbnail" />
